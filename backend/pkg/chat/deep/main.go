...
import (
	...
	"github.com/example/aichat/backend/pkg/chat/deep/agents"
	"github.com/example/aichat/backend/pkg/chat/deep/generic"
	"github.com/example/aichat/backend/pkg/chat/deep/params"
	"github.com/example/aichat/backend/pkg/chat/deep/tools"
	"github.com/example/aichat/backend/pkg/chat/deep/utils"
	"github.com/example/aichat/backend/pkg/chat/prints"
	"github.com/example/aichat/backend/pkg/chat/trace"
...
func main() {
	...
	inputFileDir = filepath.Join(wd, "playground", "test_data")
	workdir = filepath.Join(wd, "playground/input", id)
	log.Printf("inputFileDir: %s", inputFileDir)
	log.Printf("workdir: %s", workdir)

	agent, err := newExcelAgent(ctx, workdir)
	if err != nil {
		log.Fatal(err)
	}
...
func newExcelAgent(ctx context.Context, workdir string) (adk.Agent, error) {
	...
	cm, err := utils.NewChatModel(ctx,
		...
		utils.WithTemperature(float32(0.7)),
		utils.WithTopP(float32(0.5)),
		utils.WithDisableThinking(false),
	)
	if err != nil {
		return nil, err
	}
	...
	deepAgent, err := deep.New(ctx, &deep.Config{
		Name:        "ExcelAgent",
		Description: "an agent for excel task, local path: ./",
		ChatModel:   cm,
